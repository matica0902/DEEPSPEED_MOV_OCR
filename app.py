...
# 使用 threading.Semaphore 控制並發（Flask 是多線程模式）
_ocr_semaphore = threading.Semaphore(MAX_CONCURRENT_OCR)
_ocr_counter_lock = threading.Lock()
_current_ocr_count = 0

print(f"\n{'='*60}")
print(f"🖥️  MLX CPU Mode Configuration")
print(f"{'='*60}")
...
def generate_with_timeout_and_process(image, prompt, max_tokens=8192, timeout=300):
    """
    使用獨立進程執行 OCR，帶超時控制和並發限制
    
    Args:
        image: PIL Image 對象
        prompt: 提示詞
        max_tokens: 最大生成 token 數
        timeout: 超時時間（秒），CPU 模式默認 300 秒
    """
    global _current_ocr_count
    
    # 序列化圖片（在 Semaphore 外面，減少鎖定時間）
    ...
    # 獲取當前等待數量
    with _ocr_counter_lock:
        _current_ocr_count += 1
        current_count = _current_ocr_count
    
    print(f"⏳ Waiting for OCR slot... (Current queue: {current_count}, Max: {MAX_CONCURRENT_OCR})")
    
    # 等待 Semaphore 槽位（阻塞直到有空位）
    _ocr_semaphore.acquire()
    
    try:
        print(f"✅ OCR slot acquired! Starting OCR process...")
        ...
        # 檢查進程是否超時
        if process.is_alive():
            print(f"[{os.getpid()}] ⏰ OCR processing timed out after {timeout}s. Terminating subprocess {process.pid}.")
            process.terminate()
            process.join(timeout=5)
            if process.is_alive():
                process.kill()
            raise TimeoutError(f"OCR processing timeout after {timeout}s")
        ...
        print(f"🔓 OCR slot released (Remaining queue: {_current_ocr_count})")
...
def preload_model_main_process():
    ...
    try:
        print("ℹ️  Running in MLX CPU mode (Metal GPU disabled)")
